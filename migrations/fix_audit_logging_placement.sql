-- Fix: Move audit logging BEFORE RETURN statement
-- This migration ensures the audit logging code executes before the function returns
...
BEGIN
  ...
  -- ============================================================================
  -- AUDIT LOGGING - MUST BE BEFORE THE RETURN STATEMENT BELOW
  -- ============================================================================
  BEGIN
  ...
  -- Return success with token and invitation_id
  -- THIS MUST BE THE LAST STATEMENT BEFORE END
  RETURN jsonb_build_object(
    'success', true,
    'token', v_token,
    'invitation_id', v_invitation_id,
...
-- Verify the placement after running
-- Run this query to confirm:
-- SELECT 
--   CASE 
--     WHEN position('log_audit_event' in prosrc) < position('RETURN jsonb_build_object' in prosrc) 
--       THEN '✅ CORRECT: Before RETURN'
--     ELSE '❌ WRONG: After RETURN'
--   END as verification
-- FROM pg_proc
