...
export function BookingTeamCard({
  ...
  const selectedInstance = sortedInstances[selectedWeekIndex] ?? sortedInstances[0];
  ...
  return (
    ...
    >
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="px-3 py-1.5 text-sm font-semibold bg-slate-700/60 text-slate-200 rounded-md border border-slate-600/50">
                {isSingleBooking ? "Single Session" : `Block Booking (${totalInstances} sessions)`}
              </span>
            </div>
              ...
              <div className="space-y-1">
                {isSingleBooking ? (
                  <div className="text-sm">
                    ...
                    <span className="text-slate-200 font-medium">{dateDisplay}</span>
                  </div>
                ) : (
                  <div className="space-y-1">
                    <div className="text-sm">
                      <span className="text-slate-400">Start Date:</span>{" "}
                      <span className="text-slate-200 font-medium">{formatDateBritish(firstDate!)}</span>
                    </div>
                    <div className="text-sm">
                    ...
                    {showAllDates && (
                      <div className="mt-2 p-2 bg-slate-900/50 rounded border border-slate-700">
                        <div className="text-xs text-slate-400 mb-1">All Session Dates:</div>
          ...
          {weekVariations ? (
            <>
              <div className="flex items-center justify-between">
                <div className="text-xs font-semibold text-blue-300">Differences across weeks</div>
                <button
                  type="button"
              ...
              <div className="mt-2 flex flex-wrap gap-2 text-xs">
                {weekVariations.racks && (
                  <span className="px-2 py-1 rounded bg-blue-900/30 border border-blue-700/50 text-blue-200">
                    Racks vary ({weekVariations.racks.size} pattern{weekVariations.racks.size > 1 ? "s" : ""})
                  </span>
                )}
                {weekVariations.capacity && (
                  <span className="px-2 py-1 rounded bg-blue-900/30 border border-blue-700/50 text-blue-200">
                    Athletes vary ({weekVariations.capacity.size} pattern{weekVariations.capacity.size > 1 ? "s" : ""})
                  </span>
                )}
              </div>
              {showVariationDetails && (
                <div className="mt-3 space-y-2 text-xs text-blue-100">
                  ...
                  {weekVariations.capacity && (
                    <div>
                      <div className="text-blue-300 font-medium mb-1">Athletes by week</div>
                      <div className="space-y-1 ml-2">
      ...
      {!isSingleBooking && (weekVariations?.racks || weekVariations?.capacity) && selectedInstance && (
          ...
          <div className="flex items-center justify-between mb-2">
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                type="button"
                onClick={() =>
                  setSelectedWeekIndex(Math.min(weekCount - 1, selectedWeekIndex + 1))
                }
            ...
            <div>
              ...
              <span className="font-mono">
                {selectedInstance.racks.length > 0
                  ? [...selectedInstance.racks].sort((a, b) => a - b).join(", ")
                  : "None assigned"}
              </span>
            </div>
            <div>
      ...
      <div className="flex items-center gap-2 flex-wrap">
        ...
        {isPending && (
          <button
            ...
            disabled={wasEditedAfterProcessing && requiresAcknowledgment && !allChangesAcknowledged}
