...
export function useCurrentTimeIndicator(
  ...
  const currentTimePosition = useMemo(() => {
    ...
    for (let i = 0; i < timeSlots.length; i++) {
      ...
      if (currentHour > slotHour || (currentHour === slotHour && currentMinute >= slotMinute)) {
        ...
        if (i < timeSlots.length - 1) {
          ...
          if (currentHour < nextSlotHour || (currentHour === nextSlotHour && currentMinute < nextSlotMinute)) {
            slotIndex = i;
            break;
          }
        } else {
    ...
    const topPosition = headerHeight + slotIndex * rowHeight + positionInSlot * rowHeight;
