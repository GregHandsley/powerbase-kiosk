...
export function NotificationWindowSection({
  settings,
  onUpdate,
  // isUpdating,
}: Props) {
  ...
  const [localDay, setLocalDay] = useState(
    settings.notification_window_day_of_week
  );
  const [localTime, setLocalTime] = useState(
    settings.notification_window_time.slice(0, 5)
  ); // HH:mm
      ...
      <div className="flex items-start justify-between mb-4">
        <div>
          ...
          <p className="text-sm text-slate-400 mt-1">
            When bookings or changes are made after this time, email alerts will
            be sent to configured recipients. This is not a hard cutoff -
