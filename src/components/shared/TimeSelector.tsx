  ...
  return (
    <div className={clsx("space-y-1", className)}>
      ...
      <select
        value={value}
        onChange={(e) => {
          const selected = timeOptions.find((opt) => opt.time === e.target.value);
      ...
      >
        {timeOptions.map((option) => (
          <option
            ...
            className={option.isClosed ? "text-slate-500 bg-slate-800 italic" : "text-slate-100"}
          >
      ...
      {isSelectedClosed && (
        <p className="text-xs text-amber-400">
          This time is closed according to the capacity schedule. Please select a different time.
        </p>
      )}
