...
function getCapacityLimit(
  ...
  const timeStr = formatTimeSlot({ hour: time.getHours(), minute: time.getMinutes() });
...
export function useCapacityValidation(
  ...
  const { data: schedules = [] } = useQuery({
    queryKey: ["capacity-schedules-for-validation", sideId, dateRange?.earliest, dateRange?.latest],
    queryFn: async () => {
  ...
  const { data: existingInstances = [] } = useQuery({
    queryKey: ["existing-instances-for-capacity-check", sideId, dateRange?.earliest, dateRange?.latest],
    queryFn: async () => {
      if (!sideId || !dateRange) return [];
  ...
  const validationResults = useMemo(() => {
    if (!startDate || !startTime || !endTime || !sideId || schedules.length === 0) {
      return [];
    }
    ...
    return results;
  }, [startDate, startTime, endTime, sideId, schedules, existingInstances, weeks, capacity, weekCapacities]);
  ...
  const overallResult = useMemo(() => {
    ...
    const maxUsed = Math.max(...validationResults.map((r) => r.result.maxUsed), 0);
