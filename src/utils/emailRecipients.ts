...
export async function getUserEmailsByIds(userIds: string[]): Promise<Map<string, string>> {
  ...
  try {
    console.log("getUserEmailsByIds: Calling database function with", userIds.length, "user IDs");
    ...
    if (error) {
      ...
      if (error.code === "42883" || error.message?.includes("does not exist")) {
        ...
        console.error("Please run the migration: migrations/add_get_user_emails_function.sql");
      }
      
      return new Map();
    }

    console.log("getUserEmailsByIds: Received", data?.length || 0, "email addresses");
...
export async function getEmailRecipients(
  ...
  try {
    ...
    for (const role of settings.last_minute_alert_roles || []) {
      ...
      const userIds = await getUserIdsByRole(role as "admin" | "coach" | "bookings_team");
      console.log(`getEmailRecipients: Found ${userIds.length} users with role ${role}`);
      roleUserIds.push(...userIds);
    }

    // Combine role-based and specific user IDs
    const allUserIds = [...new Set([...roleUserIds, ...(settings.last_minute_alert_user_ids || [])])];
    console.log(`getEmailRecipients: Total unique user IDs: ${allUserIds.length}`);

    if (allUserIds.length === 0) {
      console.warn("getEmailRecipients: No user IDs found. Check notification settings configuration.");
      return [];
    }
    ...
    for (const userId of allUserIds) {
      ...
      if (email) {
        ...
        console.warn(`getEmailRecipients: No email found for user ID: ${userId}`);
      }
    }

    console.log(`getEmailRecipients: Returning ${emails.length} email addresses:`, emails);
    return emails;
  } catch (error) {
