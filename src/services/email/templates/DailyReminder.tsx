      ...
      <Body style={main}>
        <Container style={container}>
          ...
          <Section style={section}>
            ...
            {pendingCount === 0 && lastMinuteCount === 0 && urgentCount === 0 ? (
              <Text style={text}>
                Great news! There are no pending items requiring attention today.
              </Text>
            ) : (
              <>
                <Text style={text}>
                  You have {pendingCount} pending booking{pendingCount !== 1 ? "s" : ""} that need
                  processing.
                </Text>
                {lastMinuteCount > 0 && (
                  <Text style={text}>
                    {lastMinuteCount} last-minute change{lastMinuteCount !== 1 ? "s" : ""} require
                    your attention.
                  </Text>
                )}
                {urgentCount > 0 && (
                  <Text style={urgentText}>
                    ⚠️ {urgentCount} urgent booking{urgentCount !== 1 ? "s" : ""} in the next 7 days
