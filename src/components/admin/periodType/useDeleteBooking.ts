...
export function useDeleteBooking(
  onOverridesRefetch: () => void
) {
  const queryClient = useQueryClient();
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const deleteSelectedInstances = async (bookingId: number, instanceIds: number[]) => {
    ...
    try {
      ...
      setError(`Failed to delete bookings: ${err instanceof Error ? err.message : "Unknown error"}`);
      return false;
    } finally {
      setLoading(false);
    }
  ...
  const deleteSeries = async (bookingId: number) => {
    ...
    try {
      ...
      setError(`Failed to delete booking series: ${err instanceof Error ? err.message : "Unknown error"}`);
      return false;
    } finally {
      setLoading(false);
    }
