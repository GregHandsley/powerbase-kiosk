...
export function EmailServiceConfigSection({ settings, onUpdate, isUpdating }: Props) {
  const { user } = useAuth();
  ...
  return (
    <div className="bg-slate-900 border border-slate-700 rounded-lg p-6">
      <div className="flex items-start justify-between mb-4">
        <div>
          <h3 className="text-base font-semibold text-slate-100">Email Service Configuration</h3>
          <p className="text-sm text-slate-400 mt-1">
            Email service configuration is managed via environment variables for security.
          </p>
        </div>
      </div>
      ...
      <div className="space-y-4">
        <div className="bg-slate-800 border border-slate-700 rounded-md p-4">
          <p className="text-sm text-slate-300 mb-3">
            Configure these environment variables in your Supabase Edge Function settings:
          </p>
          <div className="space-y-2 font-mono text-xs">
            <div className="flex items-start gap-2">
              <span className="text-indigo-400">RESEND_API_KEY</span>
              <span className="text-slate-400">=</span>
              <span className="text-slate-300">Your Resend API key (starts with re_)</span>
            </div>
            <div className="flex items-start gap-2">
        ...
        <div className="pt-2 border-t border-slate-700">
          <p className="text-xs text-slate-400">
            <strong>Security Note:</strong> API keys and email configuration are stored as environment variables
            in Supabase Edge Functions, not in the database. This prevents unauthorized access through the UI.
          </p>
        </div>
        ...
        <div className="pt-4 border-t border-slate-700">
          <div className="space-y-2">
            ...
            {user?.email && (
              <p className="text-xs text-slate-400">
                Test email will be sent to: <span className="font-mono text-slate-300">{user.email}</span>
              </p>
            )}
          </div>
        </div>
