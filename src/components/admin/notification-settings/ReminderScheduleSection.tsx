...
export function ReminderScheduleSection({ settings, onUpdate, isUpdating }: Props) {
  const [localEnabled, setLocalEnabled] = useState(settings.reminder_emails_enabled);
  const [schedule, setSchedule] = useState(settings.reminder_schedule || []);
  const [selectedRoles, setSelectedRoles] = useState<string[]>(settings.reminder_recipient_roles || []);
  ...
  return (
    <div className="bg-slate-900 border border-slate-700 rounded-lg p-6">
      <div className="flex items-start justify-between mb-4">
        <div>
          <h3 className="text-base font-semibold text-slate-100">Regular Reminder Emails</h3>
          <p className="text-sm text-slate-400 mt-1">
            Schedule regular reminder emails with summaries of pending bookings and changes needing attention.
          </p>
        </div>
      </div>
      ...
      <div className="space-y-4">
        ...
        <div className="flex items-center justify-between">
          <label className="text-sm text-slate-200">Enable Reminder Emails</label>
          <button
            type="button"
            onClick={() => {
          ...
          <div className="space-y-4 pt-2 border-t border-slate-700">
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                <label className="block text-sm font-medium text-slate-200">Reminder Schedule</label>
                <button
                  type="button"
              ...
              {schedule.length === 0 ? (
                <div className="text-sm text-slate-400 text-center py-4 border border-slate-700 rounded-md">
                  No reminder schedules configured. Click "+ Add Schedule" to create one.
                </div>
              ) : (
                <div className="space-y-3">
                  {schedule.map((item, index) => (
                    <div key={index} className="border border-slate-700 rounded-md p-4 space-y-3">
                      <div className="flex items-center justify-between">
                        <span className="text-sm font-medium text-slate-200">Schedule {index + 1}</span>
                        <button
                          type="button"
                          ...
                          <select
                            ...
                            onChange={(e) =>
                              updateScheduleItem(index, {
                                ...
                                days: e.target.value === "daily" ? [1, 2, 3, 4, 5] : item.days,
                              })
                            }
                            className="w-full rounded-md border border-slate-600 bg-slate-950 px-2 py-1.5 text-sm text-slate-200 focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500"
                          >
                      ...
                      {item.frequency === "weekly" && (
                        <div>
                          <label className="block text-xs text-slate-300 mb-2">Days of Week</label>
                              ...
                              >
                                <input
                                  ...
                                  onChange={(e) => {
                                    ...
                                    updateScheduleItem(index, { days: newDays });
                                  }}
                                  className="rounded border-slate-600 bg-slate-950 text-indigo-600 focus:ring-indigo-500"
                                />
                                <span className="text-xs text-slate-300">{day.short}</span>
                              </label>
                            ))}
                          </div>
                        </div>
