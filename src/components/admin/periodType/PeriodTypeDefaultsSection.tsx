...
export function PeriodTypeDefaultsSection({
  ...
  const [editingDefault, setEditingDefault] = useState<{ periodType: PeriodType; sideKey: "Power" | "Base" } | null>(null);
  ...
  return (
    <div className="bg-slate-900 border border-slate-700 rounded-lg p-6">
      <h3 className="text-sm font-semibold text-slate-200 mb-4">Default Capacity & Platforms by Period Type</h3>
      <div className="space-y-6">
        {periodTypes.map((periodType) => {
          return (
            ...
            >
              <div className="text-sm font-medium text-slate-200 mb-4">{periodType}</div>
              <div className="grid grid-cols-2 gap-4">
                {(["Power", "Base"] as const).map((sideKey) => {
                  ...
                  const isEditing = editingDefault?.periodType === periodType && editingDefault?.sideKey === sideKey;

                  return (
                    <div key={sideKey} className="space-y-3">
                      <div className="flex items-center justify-between">
                        <div className="text-xs font-medium text-slate-300">{sideKey}</div>
                        {!isEditing && (
                          <button
                            onClick={() => {
                              setEditingDefault({ periodType, sideKey });
                              setEditingCapacity(default_?.default_capacity || 0);
                      ...
                      {isEditing ? (
                        <div className="space-y-3">
                          ...
                          <div>
                            <label className="block text-xs text-slate-400 mb-1">Platforms</label>
                            {sideId && (
                              <MiniScheduleFloorplan
                                sideKey={sideKey}
                                selectedRacks={editingPlatforms}
                                onRackClick={(rackNumber) => {
                                  const newPlatforms = editingPlatforms.includes(rackNumber)
                                    ? editingPlatforms.filter((r) => r !== rackNumber)
                                    : [...editingPlatforms, rackNumber].sort((a, b) => a - b);
                          ...
                          <div className="flex gap-2">
                            <button
                              onClick={async () => {
                                await onSave(periodType, sideKey, editingCapacity, editingPlatforms);
                        ...
                        <div className="text-xs text-slate-400 space-y-1">
                          <div>Capacity: {default_?.default_capacity ?? "Not set"}</div>
                          <div>
                            Platforms: {default_?.platforms && default_.platforms.length > 0 
