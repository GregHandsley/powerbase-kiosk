            ...
            <div>
              ...
              <p className="text-xs text-slate-400 mt-1">
                ...
                {recurrenceType === 'weekday' &&
                  'Applies to all weekdays (Monday-Friday)'}
                {recurrenceType === 'weekend' &&
                  'Applies to all weekends (Saturday-Sunday)'}
                {recurrenceType === 'weekly' && `Applies to every ${dayName}`}
                {recurrenceType === 'all_future' &&
                  `Applies to all future ${dayName}s`}
              </p>
            </div>
          )}
          ...
          <div>
            ...
            {periodType === 'Closed' ? (
                ...
                {existingCapacity ? (
                  <>
                    <div className="flex items-center gap-2">
                      ...
                      >
                        Override default capacity ({defaultCapacity}{' '}
                        {defaultCapacity === 1 ? 'athlete' : 'athletes'})
                      </label>
                    </div>
                    {useOverride && (
                      <input
                        ...
                        onChange={(e) =>
                          setCapacityOverride(
                            parseInt(e.target.value) || defaultCapacity
                          )
                        }
                        className="w-full rounded-md border border-slate-600 bg-slate-950 px-3 py-2 text-sm text-slate-100 outline-none focus:ring-1 focus:ring-indigo-500"
                      />
              ...
              <div className="text-sm text-amber-400">
                No default capacity set. Please set a default capacity for this
                period type first.
              </div>
            )}
          </div>
