// Edge Function: Export Audit Logs as CSV
// Provides CSV export of audit logs with authorization and filtering
//
// Usage: Called from admin UI to export audit logs
// Security: Requires authentication, enforces can_read_audit permission
...
interface EnrichedAuditLogRow extends AuditLogRow {
...
// Convert audit log rows to CSV with enriched data
function generateCsv(rows: EnrichedAuditLogRow[]): string {
...
serve(async (req) => {
  ...
  try {
    ...
    // Authorization: Check if user can read audit logs for this organization
    const { data: canRead, error: authError } = await supabaseClient.rpc(
      'can_read_audit',
      {
        p_org_id: orgId,
      }
    ...
    if (authError) {
      console.error('Error checking audit access:', authError);
      return new Response(
    ...
    // Parse optional filters (same as get-audit-logs)
    ...
    // Build query (same logic as get-audit-logs but with higher limit)
    ...
    const rows = filteredData as AuditLogRow[];
    ...
    const enrichedData: EnrichedAuditLogRow[] = rows.map((row) => ({
      ...row,
    ...
    console.error('Error in export-audit-logs-csv function:', error);
    return new Response(
      JSON.stringify({
        error:
