...
serve(async (req) => {
  ...
  try {
    ...
    if (!resendApiKey) {
      return new Response(
        JSON.stringify({ error: "Resend API key not configured. Set RESEND_API_KEY environment variable." }),
        {
          status: 500,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        }
    ...
    if (!emailFromAddress) {
      return new Response(
        JSON.stringify({ error: "From email address not configured. Set EMAIL_FROM_ADDRESS environment variable." }),
        {
          status: 500,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        }
    ...
    await supabaseClient.from("email_sent_log").insert({
      to_email: Array.isArray(emailRequest.to) ? emailRequest.to[0] : emailRequest.to,
      subject: emailRequest.subject,
      template_name: emailRequest.subject.toLowerCase().includes("reminder") ? "reminder" : "alert",
      metadata: {
        resend_id: data?.id,
        to: emailRequest.to,
      },
